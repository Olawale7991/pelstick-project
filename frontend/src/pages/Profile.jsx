...

    {
      ...
      <img className='w-3/6 rounded' src={userData.image} alt="" />
        ...
        <div>
          ...
          {
            isEdit ? 
            <input className='max-w-25 bg-gray-100' type="date" onChange={(e) => setUserData(prev => ({...prev,dob:e.target.value}))} value={userData.dob}/> :
        ...
        <div className='mt-10'>
          {
            isEdit ?
            <button className='border border-primary px-8 py-2 rounded-full hover:bg-primary hover:text-white transition-all duration-500' onClick={updateUserProfile}>Save Information</button> :
