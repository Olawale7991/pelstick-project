...
const {backendUrl, token, getCaregiversData} = useContext(AppContext)
  ...
  return (
    <div>
        <p className='pt-3 mt-10 font-medium text-zinc-700 border-b text-2xl'>My appointments</p>
        <div>
            {
              appointments.map((item, index)=>(
                <div className='grid grid-cols-[1fr_2fr] gap-4 sm:flex sm:gap-6 py-2 border-b' key={index}>
                  ...
                  <div className='flex flex-col gap-2 justify-end'>
                    ...
                    {item.cancelled && <button className='sm:min-w-48 py-2 border border-red-500 rounded-full text-red-500 hover:bg-red-500 hover:text-white'>Appointment cancelled</button>}
                  </div>
                </div>
              ))
            }
